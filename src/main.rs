...
use rust_bert::pipelines::sentiment::{SentimentPolarity, SentimentModel};
...
fn index(request: Json<SentimentRequest>, shared_sentiment: State<Mutex<SentimentModel>>) -> Option<Json<ResultsResponse>> {
    ...
    let results = output.iter().map(| sentiment | {
...
fn main() {
    let sentiment_classifier = SentimentModel::new(Default::default()).expect("Couldn't start model");
